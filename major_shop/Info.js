function Open_html(shop_name){
    Show_review(shop_name);
    Change_star(5);
    Tot_rating(shop_name, -1);
}
...
function Change_shopmap(shop_name){
...
function Submit(shop_name){
    ...
    if (user_review == ""){
        ...
        alert("소중한 리뷰 감사합니다.");
        Uplode_review(shop_name, user_review, user_star);
    }
}


function Uplode_review(shop_name, user_review, user_star){
    var localStorage = window.localStorage;
    var num_review = localStorage[shop_name + "tot_reviews"];
